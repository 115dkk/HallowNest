...
public class SoulVesselRelic extends CustomRelic implements ClickableRelic { // You must implement things you want to use from StSlib
    /*
    ...
    // ID, images, text.
    public static final String ID = HallownestMod.makeID("SoulVesselRelic");

    private static final Texture IMG = TextureLoader.getTexture(makeRelicPath("SoulVesselRelic.png"));
    private static final Texture OUTLINE = TextureLoader.getTexture(makeRelicOutlinePath("SoulVesselRelic.png"));
    private int EnergyStore = 1;
    private boolean isPlayerTurn = false; // We should make sure the relic is only activateable during our turn, not the enemies'.

    public SoulVesselRelic(){
        this(1);
    }


    public SoulVesselRelic(int charge) {
    ...
    public void onRightClick() {// On right click
        ...
        if (AbstractDungeon.getCurrRoom() != null && AbstractDungeon.getCurrRoom().phase == AbstractRoom.RoomPhase.COMBAT) { // Only if you're in combat
            ...
            AbstractDungeon.actionManager.addToBottom(new GainEnergyAction(EnergyStore));
