...
import Hallownest.monsters.CityofTearsEnemies.BossGrimm;
import Hallownest.monsters.CityofTearsEnemies.BossMantisLord;
import Hallownest.monsters.CityofTearsEnemies.BossSoulMaster;
...
public class CityofTearsScene extends AbstractScene {
    ...
    public CityofTearsScene() {
        super("HallownestResources/images/scene/CityofTears/atlas.atlas");
        //topBar = ImageMaster.loadImage("HallownestResources/images/scene/GreenpathBar.png");
        //what actually is this thing? does it do anything? O tried to get it to work with a custom bar but nothing happened.

        this.bg = this.atlas.findRegion("mod/CityofTearsBack");
    ...
    public void nextRoom(AbstractRoom room) {
        ...
        if (room.monsters != null) {
            for (AbstractMonster mo : room.monsters.monsters) {
                if (mo instanceof BossMantisLord) {
                    this.bg = this.atlas.findRegion("mod/MantisLordsArena");
                } else if (mo instanceof BossGrimm) {
                    this.bg = this.atlas.findRegion("mod/GrimmArena");
                } else if (mo instanceof BossSoulMaster) {
                    this.bg = this.atlas.findRegion("mod/SoulArena");
                } else {
                    this.bg = this.atlas.findRegion("mod/CityofTearsBack");
                }
            }
    ...
    //Not really sure why its all white, but ok.
    @Override
    ...
    //LMAO no idea what most this stuff does though. lol
    @Override
