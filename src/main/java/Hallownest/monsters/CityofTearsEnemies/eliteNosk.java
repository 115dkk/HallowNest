...
public class eliteNosk extends AbstractMonster {
    ...
    */
    public static final String ID = HallownestMod.makeID("Nosk");
    ...
    //you will need to assign both values whenever you spawn or setup the encounter for the monster. Most likely in your main mod file. thedefault.java
    {
        ...
        loadAnimation(
                ...
                ("HallownestResources/images/monsters/CityofTears/nosk/Nosk.atlas"),
                ("HallownestResources/images/monsters/CityofTears/nosk/Nosk.json"), 0.9F); // scale float is actually inverted because of course it is. 0.8 is bigger, 1.2 is smaller.
    ...
    private void transform ()
    {
        ...
        this.nextMove = 5;
        this.setMove((byte)5, Intent.ATTACK_DEBUFF,((DamageInfo)this.damage.get(2)).base );
        this.intent = (Intent.ATTACK_DEBUFF);
        this.createIntent();
    ...
    protected void getMove(int i)
    {
        ...
        if (AbstractDungeon.getCurrRoom().cannotLose){
            ...
            if ((i < 30) && (!lastMove((byte)0) && !lastMoveBefore((byte)0))){
                setMove((byte) 0, Intent.DEBUFF);
                return;
            } else if (( i < 50) && (!lastMove((byte)3))){
                setMove((byte) 3, Intent.DEFEND);
                return;
            } else if ((i < 80) && (!lastMove((byte)5))){
